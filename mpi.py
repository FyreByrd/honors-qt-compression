...
def compress_image_file(
    ...
    if rank != 0:
        image_data = np.empty(dims, np.uint8)
    comm.Bcast(image_data, root=0)

    inds = chunk_i(dims[0], dims[1])

    chunk = compress_and_encode_image_data(np.array(image_data[inds[0]:inds[1],inds[2]:inds[3], :]), iterations // size, detail_error_threshold)
...
def reconstruct_image_from_file(compressed_image_file: str) -> Image:
    ...
    if rank == 0:
        ...
        tmp = []
        for i in range(sz):
            tmp.append(np.hstack(chunks[i*sz:(i+1)*sz]))
        image_data = np.vstack(tmp)
