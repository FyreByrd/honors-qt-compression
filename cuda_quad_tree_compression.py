...
class QuadTreeNode:
    ...
    def draw(self, image_data: cp.array):
        if self.is_subdivided:
    ...
    def draw_self(self, image_data: cp.array):
...
class CompressNode (QuadTreeNode):
    ...
    def __init__(self, position, image_data: cp.array):
        ...
        self.detail = cp.sum(cp.std(image_data, axis=(0, 1))) * self.image_data.size

        self.color = cp.mean(image_data, axis=(0, 1)).astype(cp.uint8)
...
class ImageCompressor:
    ...
    def __init__(self, image_data: cp.array):
    ...
    def draw(self):
        new_image_data = cp.zeros(self._image_shape, dtype=cp.uint8)
...
def compress_image_data(
        ...
        detail_error_threshold: float = 10) -> cp.array:
...
def reconstruct_quadtree(data: bytes) -> ReconstructNode:
    ...
    image_data = cp.zeros((height, width, 3), dtype=cp.uint8)
    return ReconstructNode((0, 0), (width, height), subdivided_flags, colors)

def reconstruct_image_data(data: bytes) -> cp.array:
